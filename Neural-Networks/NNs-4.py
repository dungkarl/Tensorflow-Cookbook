...
# --------Activation--------
def activation2D(input_1d):
    return tf.nn.relu(input_1d)

# Create Activation Layer
my_activation_output = activation2D(my_convolution_output)


# --------Max Pool--------
def max_pool2D(input_2d, width, height, stride):
    """
    Just like 'conv2d()' above, max_pool() works with 4D arrays.
...
my_maxpool_output = max_pool2D(my_activation_output, 
                             width=maxpool_size,
                             height=maxpool_size,
...
# -------Fully Connected--------
def fully_connected2D(input_layer, num_outputs):
    """
...
my_full_output = fully_connected2D(my_maxpool_output, 5)
